...
# IMPORTANTE: 'db' se lee en vivo vía core_db.db (no `from core.database import db`,
# que capturaría el valor None de antes de que connect() lo rellene en el arranque).
import core.database as core_db
from core.database import db_insert_one, db_find
...
async def create_sintoma_registro(data: SintomaCronicoRegistro, user=Depends(get_current_user)):
    ...
    if user.get('device_id') != data.device_id:
        raise HTTPException(status_code=403, detail="No autorizado")

    # Accedemos a la colección a través del objeto db importado
    await db_insert_one(core_db.db.sintomas_cronico, data.model_dump())
    return data
