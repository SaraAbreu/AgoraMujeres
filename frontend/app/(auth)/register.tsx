...
export default function RegisterScreen() {
    ...
    const handleRegister = async () => {
        if (!name.trim()) {
            Alert.alert('Falta tu nombre', '¿Cómo quieres que te llamemos?');
            return;
        }
        if (!email.trim()) {
            Alert.alert('Falta el email', 'Introduce tu dirección de email.');
            return;
        }
        ...
        try {
            ...
            if (!res.ok) {
                console.error('BACKEND REGISTER ERROR:', data);
                throw new Error('Error registrando con backend');
            }
        ...
        >
            ...
            >
                ...
                <Text style={styles.footerText}>
                    Al registrarte aceptas nuestras condiciones de seguridad.
                </Text>
            </ScrollView>
        </KeyboardAvoidingView>
    );
...
const styles = StyleSheet.create({
    ...
    title: {
        fontSize: 26, fontWeight: '300', color: '#2D3E2D',
    ...
    divider: {
        width: 30, height: 1,
        backgroundColor: '#4A664D', opacity: 0.35, marginBottom: 10,
    },
    subtitle: {
        fontSize: 14, color: '#7A8E7A',
        fontStyle: 'italic', marginBottom: 32,
    },
    form: { width: '100%', maxWidth: 340, marginBottom: 8 },
    inputWrapper: {
        flexDirection: 'row', alignItems: 'center',
        borderWidth: 1, borderColor: 'rgba(74,102,77,0.25)',
    ...
    button: {
        ...
        backgroundColor: '#4A664D', borderRadius: 28,
        paddingVertical: 18, alignItems: 'center',
        shadowColor: '#4A664D', shadowOffset: { width: 0, height: 4 },
    ...
    footerText: {
        marginTop: 30, fontSize: 11, color: '#999', textAlign: 'center',
    },
});